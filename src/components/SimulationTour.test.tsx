...
describe('SimulationTour', () => {
  it('מסביר תחילה איזה מינוף נבדק וממחיש צבירה יומית במספרים', () => {
    ...
    const indexSequence = screen.getByLabelText('מסלול המדד: 100, אחר כך 110, ולבסוף 99')
    const leveragedSequence = screen.getByLabelText('מסלול המינוף היומי פי 3: 100, אחר כך 130, ולבסוף 91')
    expect(indexSequence).toHaveAttribute('dir', 'rtl')
    expect(leveragedSequence).toHaveAttribute('dir', 'rtl')
  ...
  it('מוביל בין חמשת ההסברים ומבדיל בזהירות בין מונטה קרלו ל־Backtest', () => {
    ...
    expect(blackSwanStep).toHaveTextContent(/אירועי קצה מופיעים בשווקים לעיתים קרובות יותר/)
    expect(blackSwanStep).toHaveTextContent(/משקיעים צריכים להביא גם אותם בחשבון/)
